...
**Updated:** 2026-04-01
**Status:** Multiple items resolved; several still pending David's confirmation
...
3. [Feature Corrections](#3-feature-corrections)
4. [Shelf Company Age-Tier Pricing Model (CONFIRMED DIRECTION)](#4-shelf-company-age-tier-pricing-model-confirmed-direction)
5. [Proposed New Add-Ons](#5-proposed-new-add-ons)
6. [Renewal Pricing Audit (STILL OPEN)](#6-renewal-pricing-audit-still-open)
...
| -------------- | ---------------- | ----------- | -------------------------------------------------------------------------------------------- |
| `sharedAddOns` | `PackageAddOn[]` | 5 [UPDATED] | Add-ons available across all packages (Virtual Office removed — now included in Silver/Gold) |
...
### Current Shared Add-Ons (5) [UPDATED]

| ID                     | Name                              | Price | Status                                                         |
| ---------------------- | --------------------------------- | ----- | -------------------------------------------------------------- |
| `foreign-registration` | Foreign Registration              | $149  | [STILL OPEN] — confirmed as add-on, price needs verification   |
| `ein`                  | EIN Obtainment                    | $75   | [CONFIRMED add-on] — per David's redline, $75 across all tiers |
| `operating-agreement`  | Custom Operating Agreement        | $99   | [STILL OPEN] — Gold-only inclusion vs. all tiers TBD           |
| `rush-filing`          | Rush Filing (24-hour)             | $199  | OK                                                             |
| `crypto-payment`       | Cryptocurrency Payment Processing | $0    | OK                                                             |

**[RESOLVED] Virtual Office REMOVED from sharedAddOns:** David confirmed Virtual Office + Mail Forwarding are INCLUDED features in all Silver and Gold packages. Virtual Office is no longer a separate add-on — it is a tier differentiator between Bronze and Silver/Gold. Remove the `virtual-office` entry from the `sharedAddOns` array and add it as an included feature in Silver/Gold package feature lists instead.
...
## 2. Add Wyoming Bronze Package [CONFLICTING]

**Status:** Was CONFIRMED in v1, now [CONFLICTING]. David says 6 core packages (excludes WY Bronze) but redline notes say WY Bronze "may become tier within Silver." Needs clarification before implementation.
...
```typescript
{
  ...
  description:
    "Basic Wyoming formation with compliance included. State filing fees, registered agent, and annual compliance at an affordable price.",
  features: [
    {
    ...
    { name: "Annual Report Filing", status: "included" },           // [UPDATED] was "add-on" — now INCLUDED per David
    { name: "Certificate of Good Standing", status: "included" },   // [UPDATED] was "add-on" — now INCLUDED per David
    { name: "Virtual Office Address", status: "not-included",
      tooltip: "Upgrade to Silver or Gold for included Virtual Office" },  // [UPDATED] explicit not-included
    { name: "Mail Forwarding (Weekly)", status: "not-included",
      tooltip: "Upgrade to Silver or Gold for weekly mail forwarding" },   // [UPDATED] explicit not-included
...
- `Annual Report Filing`: changed from `"add-on"` to `"included"` — David stated "Bronze is basic formation and compliance"
- `Certificate of Good Standing`: changed from `"add-on"` to `"included"` — same rationale
- Added `Virtual Office Address` and `Mail Forwarding (Weekly)` as `"not-included"` to maintain comparison grid alignment with Silver/Gold

### Feature Comparison: WY Bronze vs NV Bronze [UPDATED]

| Feature                       | WY Bronze [UPDATED]    | NV Bronze          |
| ----------------------------- | ---------------------- | ------------------ |
...
| Annual Report Filing          | **included** [UPDATED] | included [UPDATED] |
| **Nevada Business License**   | **N/A**                | **included**       |
| Certificate of Good Standing  | **included** [UPDATED] | included [UPDATED] |
| Virtual Office Address        | not-included           | not-included       |
| Mail Forwarding (Weekly)      | not-included           | not-included       |
| Custom Operating Agreement    | add-on                 | add-on             |

Note: WY Bronze omits "Nevada Business License" since it is Wyoming-specific. Bronze compliance features (Annual Report, Cert of Good Standing) are now INCLUDED per David's "Bronze = basic formation + compliance" definition. NV Bronze should be updated to match.
...
### 3a. EIN Obtainment [RESOLVED]

**Status:** [RESOLVED] — Confirmed as $75 add-on across ALL tiers per David's redline.

**Current prototype:** EIN is a $75 add-on for all packages (in `sharedAddOns`).

**Decision:** The prototype is CORRECT. EIN stays as a $75 add-on in `sharedAddOns`. No code changes needed.

**Old site behavior:** EIN was listed as FREE in Gold and Silver packages. David has explicitly changed this — EIN is now a paid add-on regardless of tier.

### 3b. Bank Account Opening [STILL OPEN]

**Status:** [STILL OPEN] — No decision from David yet.
...
### 3c. Mail Forwarding & Virtual Office [RESOLVED]

**Status:** [RESOLVED] — David confirmed Virtual Office + Mail Forwarding are INCLUDED in all Silver and Gold packages.

**Action required — two changes:**

**Change 1: Remove Virtual Office from `sharedAddOns`**

```typescript
// REMOVE this entry from sharedAddOns:
// {
//   id: "virtual-office",
...
**Change 2: Add as included features in Silver and Gold packages**

```typescript
// Add to features array of every Gold and Silver package (WY, NV, CA, FL)
{
  name: "Virtual Office Address",
  status: "included",
  tooltip: "Professional business address included with your package",
},
{
...
For Bronze packages, add as `not-included`:

```typescript
// Add to features array of every Bronze package
{
  name: "Virtual Office Address",
  status: "not-included",
  tooltip: "Upgrade to Silver or Gold for included Virtual Office",
},
{
  name: "Mail Forwarding (Weekly)",
  status: "not-included",
...
### Feature Order Convention [UPDATED]

Updated to include Virtual Office and Mail Forwarding in the standard feature order:
...
11. **Virtual Office Address** [UPDATED — new position]
12. **Mail Forwarding (Weekly)** [UPDATED — new position]
13. Custom Operating Agreement
14. EIN Obtainment _(add-on for all tiers)_
15. Bank Account Opening _(if confirmed — see 3b)_
...
## 5. Proposed New Add-Ons [UPDATED]

**Status:** Updated based on meeting decisions. Expedited Name Change RETIRED.
...
// Status: [STILL OPEN] — needs David's approval
...
**[RESOLVED] Expedited Name Change — RETIRED.** David stated this is "redundant anymore, could be retired." Do NOT add the following to the codebase:
...
//   price: 300,
//   description: "Change the name of your shelf company...",
// },
```
...
## 6. Renewal Pricing Audit [STILL OPEN] [UPDATED]
...
### [UPDATED] David's State Fee Data (from meeting)

David provided actual state fee numbers that affect renewal cost calculations:

| State   | Annual State Fees (approx.) | Notes                                                |
| ------- | --------------------------- | ---------------------------------------------------- |
| Wyoming | ~$100/yr                    | Relatively low; renewal pricing has more margin room |
| Nevada  | $425–$700/yr                | Significantly higher; must be factored into renewals |

**Critical issue:** NV Bronze renewal is currently $225 (LLC) — this doesn't even cover Nevada's state fees of $425–$700/yr. Either:

1. The prototype prices are Inc123 service fees only (need "+state fees" disclosure), or
2. The prototype prices are wrong and need to be increased for NV packages.
...
- [x] **EIN as $75 add-on** — prototype is already correct, no changes needed (Section 3a) [RESOLVED]
- [x] **Expedited Name Change** — do NOT add; RETIRED per David (Section 5) [RESOLVED]
- [x] **International Shipping** — do NOT add; RETIRED per David [RESOLVED]
- [ ] **Virtual Office: Remove from `sharedAddOns`** — no longer an add-on (Section 3c) [RESOLVED — ready to implement]
- [ ] **Virtual Office + Mail Forwarding: Add as INCLUDED features** in all Silver and Gold packages (Section 3c) [RESOLVED — ready to implement]
- [ ] **Bronze compliance features: Update Annual Report + Cert of Good Standing** to `"included"` in all Bronze packages (Section 2) [RESOLVED — ready to implement]

### [CONFLICTING] — Needs Clarification Before Implementation

- [ ] Add Wyoming Bronze package to `packages` array (Section 2) — David says 6 packages but redline says "may become tier within Silver"
...
### [STILL OPEN] — Pending David Confirmation

- [ ] Bank Account Opening: Add as new feature to Gold/Silver (Section 3b) [STILL OPEN]
- [ ] Operating Agreement: Confirm Gold-only inclusion or restore to all tiers [STILL OPEN]
- [ ] Foreign Registration: Confirm $149 price covers RA + state filing fees [STILL OPEN]
- [ ] Shelf Company: Implement age-tier pricing model (Section 4 — direction confirmed, prices TBD) [STILL OPEN]
- [ ] New Add-Ons: DBA Filing ($50), Domestication ($500/$750) (Section 5) [STILL OPEN]
- [ ] Renewal Pricing: Confirm all renewal amounts, especially NV vs WY differences with state fee data (Section 6) [STILL OPEN]
...
_This document reflects decisions from David's meeting on April 1, 2026. Items marked [RESOLVED] can be implemented immediately. Items marked [STILL OPEN] need David's confirmation before any code changes. Items marked [CONFLICTING] need direct clarification._
