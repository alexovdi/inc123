...
**Version:** v3 — Taxation content added
...
### 2. 50 Questions Is a Lot — Group Headers Need Visual Weight

With 50 questions across 7 categories, the page can feel overwhelming. The wireframe groups by category with H2 headers, which is correct. But the headers need enough visual weight to serve as clear section breaks — a colored left border using each pillar's identity color would do this elegantly.
...
| **Result count** | "Showing 8 of 50 questions" — updates in real time |
...
|----------|-------------|----------------|
| All (default) | Neutral | 50 |
| Privacy | Blue `#2563eb` | ~10 |
| Asset Protection | Green `#16a34a` | ~8 |
| Formation | Amber `#d97706` | ~14 |
| Compliance | Purple `#9333ea` | ~9 |
...
| 5 | "What is an offshore records custodian?" | Corporate records maintained offshore, outside U.S. subpoena jurisdiction. Unique to Incorporate123, free in Gold. | `/privacy/` |
| 6 | "Can someone find out who owns my anonymous LLC?" | With proper nominee services, your name doesn't appear in any publicly searchable state database. IRS records are not public. | `/anonymous-llc/` |
| 7 | "Is forming an anonymous LLC legal?" | Yes, completely legal. Privacy is not secrecy — you still file taxes and comply with federal reporting. | `/anonymous-llc/` |
| 8 | "How do offshore nominee directors work?" | Based outside U.S. jurisdiction, providing an additional privacy layer beyond domestic nominees. Included free since 2012. | `/privacy/` |
| 9 | "Can I use an anonymous LLC for real estate?" | Yes — common strategy. Anonymous ownership prevents public property records from linking assets to you. | `/anonymous-llc/` |
| 10 | "What's the difference between privacy and asset protection?" | Privacy hides your name. AP shields assets from lawsuits. Complementary — most clients need both. | `/asset-protection/` |
...
| 11 | "How does entity type affect my taxes?" | Entity type determines default tax treatment: single-member LLCs are sole props, multi-member LLCs are partnerships, corps are C-Corps unless S-Corp elected. | `/formation/entity-tax-guide` |
| 12 | "When should I elect S-Corp taxation?" | Generally when net income consistently exceeds $80K/year. Below that, added accounting costs outweigh SE tax savings. | `/formation/entity-tax-guide` |
| 13 | "Does forming in Wyoming eliminate my state taxes?" | WY has no state income tax, but operating in another state may trigger that state's taxes. CA has $800 franchise tax minimum. | `/formation/entity-tax-guide` |
| 14 | "Pass-through vs. double taxation?" | Pass-through (LLCs, S-Corps): taxed once on personal return. Double taxation (C-Corps): corporate tax + dividend tax. Most small businesses prefer pass-through. | `/compare/llc-vs-scorp-vs-ccorp-tax` |

### Compliance Questions (9)
...
| 7 | "Does my Wyoming LLC owe Wyoming business taxes?" | No. WY has no state income tax, no LLC franchise tax, no gross receipts tax. Only the $60 annual report fee. | `/compliance/tax-obligations` |
| 8 | "What is California's $800 franchise tax?" | CA imposes $800/year minimum on every LLC formed or registered there, regardless of income. Applies to WY LLCs foreign registered in CA. | `/compliance/tax-obligations` |
| 9 | "Do I need quarterly estimated tax payments?" | If you expect to owe $1,000+ in federal taxes for the year, yes. Due April 15, June 15, September 15, January 15. | `/compliance/tax-obligations` |
...
| 2 | "Where is your office?" | Our mailing address is in Reno, Nevada. We offer same-day filing with the Nevada Secretary of State. | `/about/` |
...
| Formation | "LLC vs Corporation →" `/llc-vs-corporation/` | "Entity Tax Guide →" `/formation/entity-tax-guide` |
| Compliance | "Registered Agent Services →" `/registered-agent/` | "Tax Filing & Obligations →" `/compliance/tax-obligations` |
