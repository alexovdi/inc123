...
- Set via `--port 2121` in `dev` and `start` scripts in `package.json`
- Before starting work, ensure the dev server is running. If not:
  ...
  2. Clear stale cache: `rm -rf .next`
  3. Start fresh: `npm run dev`
  4. Verify: confirm HTTP 200 on `http://localhost:2121`
- NEVER clear `.next` while the dev server is still running — it causes 500 errors
- If the dev server hangs during compilation, kill the process and restart (don't wait)
...
- **Dynamic routes** use `generateStaticParams()` for SSG + `generateMetadata()` for SEO
- **Server → Client delegation:** server page finds data, passes to `<XxxClient>` component
- **Layout spacing:** all pages use `max-w-content` + `px-container-x` + `py-section-y`
- **Invalid slugs:** use `notFound()` from `next/navigation` for missing data
...
- **Server components** (pages) never use client directives
