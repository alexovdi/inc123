...
/* ------------------------------------------------
   Static params — 5 privacy cluster slugs
   ------------------------------------------------ */
const PILLAR = "privacy" as const;
const PILLAR_LABEL = "Business Privacy";
const PILLAR_HREF = "/privacy";
...
/* ------------------------------------------------
   Page Component
   ------------------------------------------------ */
export default async function PrivacyClusterPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const suggestions = [
    ...cluster.sidebarLinks.slice(0, 3).map((link) => ({
      title: link.title,
      description: `Learn more about ${link.title.toLowerCase()} and how it relates to your privacy strategy.`,
      href: link.href,
      pillar: PILLAR,
    })),
  ];
