# Incorporate123 — Homepage Component Inventory
...
Atomic UI elements with CVA (Class Variance Authority) variants for size, color, and state. None are homepage-exclusive — every primitive is used across multiple templates.

### P1. Button

| Property | Specification |
|----------|--------------|
| **Variants** | `primary` (amber), `secondary` (ghost/outline), `ghost` (transparent + underline on hover) |
| **Sizes** | `sm`, `md`, `lg` |
...
| Property | Specification |
|----------|--------------|
| **Variants** | `default` (blue), `muted` (gray — safety net style), `pillar` (inherits pillar color) |
...
| Property | Specification |
|----------|--------------|
| **Variants** | `highlight` (amber bg — "Most Popular"), `pillar` (pillar color bg), `trust` (gray/navy bg) |
| **Sizes** | `sm`, `md` |
...
| **Color** | Inherits from parent or accepts explicit color prop |
...
| Property | Specification |
|----------|--------------|
| **Variants** | `display-xl` (48px Outfit), `display-lg` (36px), `display-sm` (24px), `section` (20px) |
| **Color** | Default foreground, or `light` (white for dark backgrounds) |
...
| Property | Specification |
|----------|--------------|
| **Variants** | `lg` (18px lead), `base` (16px body), `sm` (14px caption), `xs` (12px legal) |
| **Color** | `default`, `muted`, `light` (white on dark bg) |
...
| Property | Specification |
|----------|--------------|
| **Variants** | `subtle` (1px light gray), `section` (used between major page sections), `pillar` (pillar-colored) |
...
| Property | Specification |
|----------|--------------|
| **Behaviors** | Sticky on scroll (compact mode with shadow), mega menu trigger on hover/tap |
...
| **Pillar color** | Each column gets subtle top border accent in its pillar color |
...
| **Background** | Deep Navy full-width with abstract privacy-themed graphic |
...
| Property | Specification |
|----------|--------------|
| **Contains** | Pillar color top border (4px), Icon (pillar-tinted), Badge (pillar label), BodyText (core message), BodyText (trust element, italic), TextLink (soft CTA) |
...
| **Highlighted state** | Double border or shadow, slight scale increase, elevated z-index, badge visible |
...
| Property | Specification |
|----------|--------------|
| **Contains** | Icon (lg, white/light), Heading (section size, white), BodyText (sm, light) |
| **Background** | Inherits from parent section (Deep Navy) — no individual card background |
...
| **Section backgrounds** | Alternating: Navy (hero) → White → White → Navy (differentiators) → Light gray (trust) → White → White → Navy (final CTA) → Dark (footer) |
| **Spacing** | Section-level padding: 80px top/bottom desktop, 48px mobile. Define as design token `--section-padding`. |
...
| **Sticky** | Fixed to top on scroll, compact mode (reduced height + shadow) |
...
Each component references design tokens rather than hardcoded values. This section confirms which token categories each component needs.

| Component | Colors | Typography | Spacing | Shadows | Animation |
|-----------|--------|-----------|---------|---------|-----------|
| P1 Button | ✓ brand + pillar | ✓ | ✓ | — | ✓ hover/active |
| P3 Badge | ✓ brand + pillar | ✓ | ✓ | — | — |
| P7 PriceDisplay | ✓ brand | ✓ mono | ✓ | — | — |
| C2 MegaMenu | ✓ pillar | ✓ | ✓ | ✓ dropdown | ✓ open/close |
| C4 HeroSection | ✓ brand (navy bg) | ✓ display | ✓ section | — | ✓ fade-in (optional) |
| C5 PillarCard | ✓ pillar | ✓ | ✓ | ✓ hover lift | ✓ hover |
| C6 PackagePreviewCard | ✓ brand + amber | ✓ + mono | ✓ | ✓ highlight | ✓ hover |
| C8 TrustSignalsBar | ✓ brand | ✓ | ✓ | — | — |
| C12 TestimonialCarousel | ✓ brand | ✓ | ✓ | — | ✓ slide transition |
| C13 StickyMobileCTA | ✓ amber | ✓ | ✓ | ✓ bar shadow | ✓ appear on scroll |

**All components** depend on the Brand Colors and Typography token categories. The design system creation task (8 hours) must define these tokens before any component can be visually designed.
