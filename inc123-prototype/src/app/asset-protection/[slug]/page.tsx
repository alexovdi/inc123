...
/* ------------------------------------------------
   Static params — 5 asset-protection cluster slugs
   ------------------------------------------------ */
const PILLAR = "asset" as const;
const PILLAR_LABEL = "Asset Protection";
const PILLAR_HREF = "/asset-protection";
...
/* ------------------------------------------------
   Page Component
   ------------------------------------------------ */
export default async function AssetProtectionClusterPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const suggestions = [
    ...cluster.sidebarLinks.slice(0, 3).map((link) => ({
      title: link.title,
      description: `Learn more about ${link.title.toLowerCase()} and how it fits your asset protection strategy.`,
      href: link.href,
      pillar: PILLAR,
    })),
  ];
